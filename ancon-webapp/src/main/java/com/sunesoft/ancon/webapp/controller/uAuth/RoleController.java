...
//        return view("uAuth/_deleteRoleForm", model);
    ...
    public ModelAndView roleInfos(Model model) {
        model.addAttribute("ucheck",5);
        return view(layout, "uAuth/role", model);
    }
    ...
    public void addOrUpdateUser(HttpServletRequest request, HttpServletResponse response) {
        ...
        CommonResult result = sysRoleService.addOrUpdate(roleDto) > 0 ? new CommonResult(true) : new CommonResult(false,"角色名已经存在");
    ...
    @ResponseBody
    public CommonResult deleteRoleManager(HttpServletRequest request){
        String id = request.getParameter("id");
